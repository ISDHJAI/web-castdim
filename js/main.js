...
function waLink(msg, num) {
  return "https://wa.me/" + (num || CASTDIM.whatsapp) + "?text=" + encodeURIComponent(msg || CASTDIM.defaultMsg);
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  /* enlaces de WhatsApp con mensaje personalizado: <a data-wa="mensaje">
     · data-wa-num="521..." dirige ese botón a un número distinto al general */
  document.querySelectorAll("[data-wa]").forEach((el) => {
    el.setAttribute("href", waLink(el.dataset.wa, el.dataset.waNum));
  ...
  if (form) {
    form.addEventListener("submit", (ev) => {
      ...
      window.open(waLink(msg, form.dataset.waNum), "_blank");
    });
  }

  /* modales de desarrollo: <a data-modal-open="id-del-modal"> */
  const closeModal = (ov) => {
  ...
  document.querySelectorAll(".modal-overlay").forEach((ov) => {
    ov.addEventListener("click", (ev) => {
      if (ev.target === ov || ev.target.closest(".modal__close")) closeModal(ov);
    });
  });
